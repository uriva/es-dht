...
DHT.prototype = {
  ...
  start_lookup: function (id, number) {
    ...
    if (bucket["has"](id)) {
      ...
      bucket["del"](this._id);
      max_fraction = Math.max(
        this._fraction_of_nodes_from_same_peer,
        1 / this._peers["count"](),
      );
   ...
   */ set_peer: function (peer_id, peer_state_version, proof, peer_peers) {
    var expected_number_of_items,
      proof_block_size,
      expected_proof_height,
      proof_height,
      last_block,
      i$,
      to$,
    ...
    detected_peer_id = this["check_state_proof"](
      peer_state_version,
      proof,
      peer_id,
    );
   ...
   */ _get_state: function (state_version) {
    ...
    if (
      !state_version ||
      are_arrays_equal(state_version, this._latest_state[0])
    ) {
   ...
   */ get_state_proof: function (state_version, peer_id) {
    ...
    if (
      !state ||
      (!state.has(peer_id) && !are_arrays_equal(peer_id, this._id))
    ) {
   ...
   */ check_state_proof: function (state_version, proof, node_id) {
    if (
      proof[0] === 0 &&
      merkleTreeBinary["check_proof"](state_version, proof, node_id, this._hash)
    ) {
